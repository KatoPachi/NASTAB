#' ### {fixest}の回帰分析表の作成
#'
#' パネルデータモデルを実行する`fixest::feols`の結果を`kable`に渡しやすいように文字列のdataframeに変換する
#'
#' @param fixestobj `fixest`クラスのオブジェクトで構成されているリスト
...
#'
#+
regtab_fixest <- function(
  fixestobj, keep_coef = NULL, rm_coef = NULL, label_coef = NULL,
  bindtab = FALSE) {

  # make coeftab: coefficient, star, and standard errors in parenthesis
  coeftab <- fixestobj %>%
    purrr::map(~ summary(.)$coeftable) %>%
