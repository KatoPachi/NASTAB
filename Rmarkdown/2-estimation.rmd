...
```{r SummaryOutcome2, fig.cap = "Average Logged Giving in Three Income Groups. Notes: We created three income groups, with the relative price of giving rising (circle), unchanged (triangle), and falling (square) between 2013 and 2014.", out.width = "85%", out.extra = ""}
df %>%
...
The instrumental variable $\text{WageEarner}_{it}$ must hold independence of $u_{it}$ conditional on covariates.[^corfe]
...
[^corfe]: We allow arbitrary correlation between instrument and fixed effects.
...
\begin{equation}
  R_{it} = 1[\delta_0 + Z_{it} \delta_1 + u_{it1} > 0], (\#eq:stage1)
\end{equation}
...
The second approach estimates equation \@ref(eq:stage2) with 2SLS with fixed effects,
...
\begin{equation}
    ...
    + X_{it}\beta +\mu_i +\iota_t +u_{it}. (\#eq:stage2red)
\end{equation}
