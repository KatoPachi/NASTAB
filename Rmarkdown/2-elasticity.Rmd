...
```{r MainOverall, eval = doctype == "draft"}
overall %>%
  regtab_fixest(
...
```{r MainIntensive, eval = doctype == "draft"}
intensive %>%
  regtab_fixest(
    keep_coef = c("log_price", "log_pinc_all"),
    label_coef = list(
...
```{r MainExtensive, eval = doctype == "draft"}
...
## Result of Applicable Peice Elasticitiy

Extensive-margin price elascitiy is more elastic than intensive-margin price elasticity
```

```{r, eval = doctype == "slide"}
tabwald <- ext_wald %>%
  dplyr::filter(vars == "Implied price elasticity") %>%
  dplyr::mutate(
    vars = dplyr::recode(
      vars,
      "Implied price elasticity" = "$\\hat{\\delta}/\\bar{D}$"
    )
  ) %>%
  dplyr::select(vars, stat, reg5) %>%
  dplyr::rename(reg3 = reg5)

tabx <- with(
  xlist_tab,
  tibble::tibble(
    vars = vars, stat = stat, reg1 = reg5, reg2 = reg5, reg3 = reg5
  )
)

tab1 <- list(overall$reg5, intensive$reg5) %>%
  regtab_fixest(
    keep_coef = "log_price",
    label_coef = list(log_price = "$\\hat{\\varepsilon}_p^{int}$")
  )

tab2 <- list(extensive$reg5) %>%
  regtab_fixest(
    keep_coef = "log_price",
    label_coef = list(log_price = "$\\hat{\\delta}$")
  ) %>%
  purrr::map(~ dplyr::rename(., reg3 = reg1))

list(
  coef = dplyr::full_join(tab1$coef, tab2$coef, by = c("vars", "stat")),
  stat = dplyr::full_join(tab1$stat, tab2$stat, by = c("vars", "stat"))) %>%
  regtab_addline(list(tabwald, tabx)) %>%
  dplyr::mutate(reg3 = if_else(vars == "ln(giving price)", "", reg3)) %>%
  dplyr::mutate(vars = if_else(stat == "se", "", vars)) %>%
  dplyr::select(-stat) %>%
  kable(
    col.names = c("", "Overall", "Intensive", "Extensive"),
    align = "lccc",
...
and the last price is equivalent to the first one in the case of the tax credit system.[^res1stage]
...
[^res1stage]: In fact, in any specifications, the coefficient of the first price is greater than 0.952, <!-- check iv1coef --> and its F-statistics is greater than 6425.96. <!-- check iv1wald --> We do not show the first stage results of the panel IV using the first price as an insrument.
```

```{r LastOverall, eval = doctype == "draft"}
overall_iv <- xlist %>%
  purrr::map(~ fit_fixest(
    y = log_total_g ~ .,
    x = update(., ~ . - log_price),
...
```{r LastIntensive, eval = doctype == "draft"}
intensive_iv <- xlist %>%
  purrr::map(~ fit_fixest(
    y = log_total_g ~ .,
    x = update(., ~ . - log_price),
...
```{r LastExtensive, eval = doctype == "draft"}
extensive_iv <- xlist %>%
  purrr::map(~ fit_fixest(
    y = i_ext_giving ~ .,
    x = update(., ~ . - log_price),
...
```{r ShortOverall, eval = doctype == "draft"}
overall_s1 <- xlist2 %>%
  purrr::map(~ fit_fixest(
    y = log_total_g ~ ., x = .,
    fixef = ~ year + pid, cluster = ~ pid,
...
```{r ShortIntensive, eval = doctype == "draft"}
intensive_s1 <- xlist2 %>%
  purrr::map(~ fit_fixest(
    y = log_total_g ~ ., x = .,
    fixef = ~ year + pid, cluster = ~ pid,
...
```{r ShortExtensive, eval = doctype == "draft"}
extensive_s1 <- xlist2 %>%
  purrr::map(~ fit_fixest(
    y = i_ext_giving ~ ., x = .,
    fixef = ~ year + pid, cluster = ~pid,
...
```{r kdiffOverall, eval = doctype == "draft"}
overall_kdiff <- kdiff %>%
  purrr::map(~ fit_fixest(
    y = .$y,
    x = update(
...
```{r kdiffIntensive, eval = doctype == "draft"}
