...
These results are obtained by ITT approach.
...
Since the tax reform affects not only relative price of giving but also decision-making of a tax deduction,
previous results have some policy implications.
If we explicitly control the self-selection of a tax deduction,
we may obtain further policy implications about tax incentive of charitable giving.
In this section, we report results of the panel IV method which controlls the self-selection of a tax deduction.
```
...
```{asis, echo = doctype == "draft"}
Before describing the strategy of instrumental variable in detail,
...
\@ref(tab:SummaryDeduct) shows the number of tax deduct among donors in our sample.[^year18]
When we only use information about tax deduction in the case of labor income,
the majority of respondents did not apply for a tax deduction (see the first and third column).
...
As a result, more than half of respondents applied for a tax deduction.

[^year18]: Due to small sample, we rule out observations in 2018.
```
...
However, as shown in \@ref(tab:SummaryDeduct),
applying for a tax deduction may incur some costs because 
some respondents did not applied for a tax deduction.
Employment status is one dimension of variation of applyed cost.
Those who are employed in a firm can ask a firm to apply for a tax deduction instead,
while those who are self-employed needs to apply for it by themselves.
In fact, \@ref(fig:DeductByEmployee) shows that
the proportion of applying for a tax deduction among employee is higher than among others.
Thus, we use employment status as an instrument of a tax deduction.

Our estimation strategy is the panel IV method.
...
The variable $\ln p^f_{it}$ is a first price of charitable giving
to avoid the problem that giving price depends on charitable giving.
...
If this is true, then the employment status affects charitable giving through income effect.
Second, an opprtunity of giving may depend on employment status.
For example, a doctor and a public servant may have more opprtunities to donate than others.
...
As a result, employed dummy is strongly positively correlated with applying for a tax deduction.
In any specification, employed dummy increase the probability of tax deduction by more than 45 percent points.
Moreover, the ratio of coefficient to standard error is greater than nine.
This implies that F-statistics of instrument is greater than 81.
Thus, our panel IV method does not have a weak instument problem.
For the second stage, we obtain the prediceted value of $R_{it}$, using the model (3).
```
...
```{asis, echo = doctype == "draft"}
Table \@ref(tab:OverallReport) reports overall elasiticties controlling the self-selection of tax report.
When we only include individual and time fixed effects, the overall elasticity is -2.364.
When we control same covariates as the model (5) in Table \@ref(tab:OverallMain),
the overall elasticity is -1.710,
which is more elastic than the baseline results shown in Table \@ref(tab:OverallMain).
We can naturally interpret this result.
The relative price of giving among those who did not apply for a tax deduction
is always one regardless of donations and income.
The standard economic theory predicts that
an amount of donations among those who did not apply for a tax deduction is lower than
among those who did.
Thus, when we see those who did not apply for a tax deduction as those who did,
the ITT approach, shown in Table \@ref(tab:OverallMain), underestimates the true price effect.
```
