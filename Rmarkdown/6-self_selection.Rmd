...
## Instrumental Variable of Declaration of a Tax Releif
...
```{r SummaryDeduct, eval = doctype == "draft"}
df %>%
  ...
  kable(
    caption = "Descriptive Statistics of Declaration among Donors",
    col.names = c(
  ...
  add_header_above(
    c("", "Declaring a tax relief" = 2, "Not declaring a tax relief" = 2)
  ) %>%
...
```{r DeductByEmployee, fig.cap="Proportion of declaring a tax relief Grouped by Employment Status. Notes: We restrict sample to those who donated.", eval = doctype == "draft"}
df %>%
  dplyr::filter(!is.na(employee)) %>%
  dplyr::filter(year < 2018 & i_total_giving > 0) %>%
  group_by(year, employee) %>%
...
```{r stage1Report, eval = doctype == "draft"}
1:2 %>%
  purrr::map(
    function(x)
    est_report[[x]] %>%
  ...
  kable(
    caption = "First-Stage Result: Effect of Employee on Declaration",
...
```{r OverallReport, eval = doctype == "draft"}
...
```{r IntensiveReport, eval = doctype == "draft"}
...
```{r ExtensiveReport, eval = doctype == "draft"}
...
## Result of Effective Price Elasticity
...
tabwald <- waldtest_ext %>%
  ...
  dplyr::select(vars, stat, reg6) %>%
  dplyr::rename(reg3 = reg6)

tabx <- with(
  xlist_tab,
  tibble::tibble(
    vars = vars, stat = stat, reg1 = reg3, reg2 = reg3, reg3 = reg3
  )
)

tab1 <- list(ivest_report2[[3]], ivest_report2_int[[3]]) %>%
  regtab_fixest(
    keep_coef = "log_price",
    label_coef = list(
      "log_price:p_ext_benefit_tl" = "$\\hat{\\varepsilon}_p^{int}$"
    )
  )

tab2 <- list(ivest_report2_ext[[3]]) %>%
  regtab_fixest(
    keep_coef = "log_price",
    label_coef = list(
      "log_price:p_ext_benefit_tl" = "$\\hat{\\delta}$"
    )
  ) %>%
