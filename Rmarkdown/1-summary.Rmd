...
```{r SummaryOutcome, out.width="100%", fig.cap="Proportion of Donors and Average Donations among Donors. Notes: The left and right axises respectively mesure proportion of donors and the average amount donations among donors. Authors made this graph based on NasTaB data.", eval = doctype == "draft"}
avgext <- df %>%
  group_by(year) %>%
...
```{r SummaryPriceChange, out.width="85%", fig.cap="Income Distribution and Giving Price in 2013"}
df %>%
  filter(year == 2013) %>%
