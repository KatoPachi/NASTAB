...
```{r}
use2$data <- use2$data %>%
  mutate(inc_group = if_else(tinc < 2200, "Low", "High")) %>%
  ungroup()

dta <- use2$first_price()$data
```

## Intensive-Margin Price Elasticities

```{r}
mod <- outcome ~ ..stage2 | effective ~ applicable

fit <- dta %>%
  group_by(type, inc_group) %>%
  nest() %>%
  mutate(
    mu = map_dbl(data, ~ with(., mean(outcome, na.rm = TRUE))),
    fit = map(data, ~ feols(mod, data = ., vcov = ~ hhid))
  ) %>%
  arrange(desc(type), desc(inc_group)) %>%
  select(-data)
...
addtab <- fit %>%
  ...
  mutate(id = paste0(type, "_", inc_group)) %>%
  ungroup() %>%
  select(-type, -inc_group) %>%
  pivot_longer(-id) %>%
...
addtab <- bind_rows(implied_e, stat_stage1_tab)
attr(addtab, "position") <- 5:8
