...
use$data %>%
  ...
  with(prop.table(table(bracket13))) %>%
  as.data.frame() %>%
  tibble() %>%
  mutate(
    pre_price = case_when(
      bracket13 == "(A) --1200" ~ 0.94,
      bracket13 == "(B) 1200--4600" ~ 0.85,
      bracket13 == "(C) 4600--8800" ~ 0.76,
      bracket13 == "(D) & (E) 8800--30000" ~ 0.65
    ),
    change_price = 100 * (0.85 - pre_price) / pre_price,
...
b <- dt %>%
  dplyr::filter(year == 2013) %>%
  with(table(d_relief_donate, lead_claim, bracket13)) %>%
  as.data.frame() %>%
  mutate(
    Freq = Freq / sum(Freq),
    pre_price = case_when(
    ...
    post_price = case_when(
      lead_claim == 0 ~ 1,
      lead_claim == 1 ~ 0.85
    ),
    change_price = 100 * (post_price - pre_price) / pre_price,
