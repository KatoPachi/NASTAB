...
同様の結果が得られた。
列（５）より弾力性は以下のように計算される。

$$
	\hat{\eta}_{health} = 3.142 - 2 * 0.333 * \log G_{jt}
$$

予測した弾性値と一人当たり政府支出の関係を示した散布図を以下に示す。
その結果、社会福祉分野支出の弾性値と比べて、絶対値の値は小さい（比較的に非弾力的）である。
...
はじめに、`welfare_level_3scale`でグループ分けをして、政府支出の時系列変化を示す。
...
$$
\text{welfare_level_3scale}_{ijt} = \alpha + \beta \cdot \text{log_PPP_pubbdg}_{jt} + \gamma X_{ijt} + \mu_i + \lambda_t + \theta_j + \epsilon_{ijt}.
$$
...
```{r, echo = FALSE}
htmltools::includeHTML("C:/Users/vge00/Desktop/nastab/_assets/robustreg2.html")
```