...
```{r kableEstimateElasticityPart1}
...
```{r kableEstimateElasticityPart2Slide1, eval = params$slide}
...
```{r kableEstimateElasticityPart2Slide2, eval = params$slide}
...
```{r kableEstimateElasticityPart2, eval = !params$slide}
...
fstat_line <- c(vars = "F-statistics of IV", stat = "stat", sprintf("%1.2f", fstat)) %>% 
  setNames(c("vars", "stat", paste("reg", 1:length(fstat), sep = "")))
...
# tabulation
# f-stat (first stage)
e_fstat <- e_elast_rob1$result %>% purrr::map(~.$stage1$iv1fstat$log_lprice[["F"]]) %>% as_vector()
e_fstat_line <- c(vars = "F-statistics of IV", stat = "stat", sprintf("%1.2f", e_fstat)) %>% 
  setNames(c("vars", "stat", paste("reg", 1:length(e_fstat), sep = "")))
...
# tabulation
# f-stat (first stage)
i_fstat <- e_elast_rob1$result %>% purrr::map(~.$stage1$iv1fstat$log_lprice[["F"]]) %>% as_vector()
i_fstat_line <- c(vars = "F-statistics of IV", stat = "stat", sprintf("%1.2f", i_fstat)) %>% 
  setNames(c("vars", "stat", paste("reg", 1:length(i_fstat), sep = "")))
...
```{r kableLastElasticity1Slide, eval = params$slide}
...
```{r kableLastElasticity2Slide1, eval = params$slide}
...
```{r kableLastElasticity2Slide2, eval = params$slide}
...
```{r kableLastElasticity1, eval = !params$slide}
newtab.elast_rob1 %>% 
	mutate(vars = if_else(stat == "se", "", vars)) %>% 
	dplyr::select(-stat) %>%
	knitr::kable(
...
```{r kableLastElasticity2, eval = !params$slide}
...
```{r kableShortElasticity1Slide, eval = params$slide}
...
```{r kableShortElasticity2Slide1, eval = params$slide}
...
```{r kableShortElasticity2Slide2, eval = params$slide}
...
```{r kableShortElasticity1, eval = !params$slide}
tab.elast_rob2 %>% 
	mutate(vars = if_else(stat == "se", "", vars)) %>% 
	dplyr::select(-stat) %>%
	knitr::kable(
...
```{r kableShortElasticity2, eval = !params$slide}
...
```{r kablekDiffElasticitySlide1, eval=params$slide}
...
```{r kablekDiffElasticitySlide2, eval = params$slide}
...
```{r kablekDiffElasticity, eval = !params$slide}
