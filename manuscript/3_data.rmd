...
df %>% 
	...
	dplyr::select(vars, N, mean, sd, min, max) %>%    #for slides
	knitr::kable(
    format = "latex",
    caption = "Summary Statistics",
    # col.names = c("", "N", "Mean", "Std.Dev.", "Min", "p25", "Median", "p75", "Max"),  # for papers
     col.names = c("", "N", "Mean", "Std.Dev.", "Min", "Max"),  # for slides
    row.names = FALSE,
    align = "lccc", 
...
```{asis, echo = params$slide}
## Income and Giving Price (Cont'd)
...
```{r SummaryOutcome, out.width="90%", fig.cap="Proportion of Donors and Average Donations among Donors"}
avgext <- df %>% 
	group_by(year) %>% 
