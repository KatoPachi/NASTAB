...
Elasticity of extensive margin can be estimated by  

$$
    D_{ijt} = \alpha_i + \beta_1 \log(\text{Price}_{ijt}) + \delta X_{ijt} + \lambda_t + \epsilon_{ijt}.
$$

- $D_{ijt$ is a dummy variable taking 1 if individual $i$ donated in year $t$.

The extensive-margin price elasticity can be calculated by $\beta_1/\bar{D}$
where $\bar{D}$ is the sample mean of $D_{ijt}$
...
## Result of Extensive Margin 

```{r shapeEstimateElasticityExtensive}
org.basereg <- read.dta13("_assets/EstimateElasticityExtensive.dta") %>% data.frame()
...
tab.basereg <- rbind(as.matrix(coef.basereg), as.matrix(elas.basereg)) %>% 
	rbind(stat.basereg) %>% 
	data.frame()
```

```{r kableEstimateElasticityExtensive, include = TRUE}
knitr::kable(
    tab.basereg,
    format = "latex",
    caption = "Extensive-Margin Price Elasticity",
    col.names = c("", "(1)", "(2)", "(3)", "(4)", "(5)"),
    row.names = FALSE,
    align = "lccccc", 
    booktabs = TRUE, escape = TRUE, linesep = ""
...
- Following Alumina et al. (2020), we estimated the model (5) in the previous slide, using the panel IV model for $k = 1, 2, 3$.
...
## Result of Robustness Check 1: Extensive Margin

## Result of Robustness Check 1: Intensive Margin
...
## Result of Robustness Check 2: Extensive Margin

## Result of Robustness Check 2: Intensive Margin
...
## Five Subgroup Regression by Political Trust
...
## Three Subgroup Regression by Trust Index
